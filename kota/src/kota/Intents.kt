...
inline fun Context.startActivityResolved(intent: Intent, fallback: () -> Unit): Unit =
        if (intent.isResolvable(this)) startActivity(intent)
        else fallback()

/** Starts an activity that can handle this [intent], otherwise [fallback] will be executed. */
inline fun Activity.startActivityResolved(intent: Intent, fallback: () -> Unit): Unit =
        if (intent.isResolvable(this)) startActivity(intent)
        else fallback()

/** Starts an activity that can handle this [intent], otherwise [fallback] will be executed. */
inline fun Fragment.startActivityResolved(intent: Intent, fallback: () -> Unit): Unit =
...
@RequiresApi(16)
inline fun Context.startActivityResolved(intent: Intent, options: Bundle, fallback: () -> Unit): Unit =
        if (intent.isResolvable(this)) startActivity(intent, options)
        else fallback()

/** Starts an activity that can handle this [intent] with options, otherwise [fallback] will be executed. */
@RequiresApi(16)
inline fun Activity.startActivityResolved(intent: Intent, options: Bundle, fallback: () -> Unit): Unit =
        if (intent.isResolvable(this)) startActivity(intent, options)
        else fallback()

/** Starts an activity that can handle this [intent] with options, otherwise [fallback] will be executed. */
@RequiresApi(16)
inline fun Fragment.startActivityResolved(intent: Intent, options: Bundle, fallback: () -> Unit): Unit =
...
inline fun Context.startActivitiesResolved(intents: Array<Intent>, fallback: () -> Unit): Unit =
        if (intents.all { it.isResolvable(this) }) startActivities(intents)
        else fallback()

/** Starts activities that can handle each [intents], otherwise [fallback] will be executed. */
inline fun Activity.startActivitiesResolved(intents: Array<Intent>, fallback: () -> Unit): Unit =
        if (intents.all { it.isResolvable(this) }) startActivities(intents)
        else fallback()

/** Starts activities that can handle each [intents] with options, otherwise [fallback] will be executed. */
@RequiresApi(16)
inline fun Context.startActivitiesResolved(intents: Array<Intent>, options: Bundle, fallback: () -> Unit): Unit =
        if (intents.all { it.isResolvable(this) }) startActivities(intents, options)
        else fallback()

/** Starts activities that can handle each [intents] with options, otherwise [fallback] will be executed. */
@RequiresApi(16)
inline fun Activity.startActivitiesResolved(intents: Array<Intent>, options: Bundle, fallback: () -> Unit): Unit =
        if (intents.all { it.isResolvable(this) }) startActivities(intents, options)
        else fallback()

/** Starts an activity for result that can handle this [intent], otherwise [fallback] will be executed. */
inline fun Activity.startActivityForResultResolved(intent: Intent, requestCode: Int, fallback: () -> Unit): Unit =
        if (intent.isResolvable(this)) startActivityForResult(intent, requestCode)
        else fallback()

/** Starts an activity for result that can handle this [intent], otherwise [fallback] will be executed. */
inline fun Fragment.startActivityForResultResolved(intent: Intent, requestCode: Int, fallback: () -> Unit): Unit =
...
@RequiresApi(16)
inline fun Activity.startActivityForResultResolved(intent: Intent, requestCode: Int, options: Bundle, fallback: () -> Unit): Unit =
        if (intent.isResolvable(this)) startActivityForResult(intent, requestCode, options)
        else fallback()

/** Starts an activity for result that can handle this [intent] with options, otherwise [fallback] will be executed. */
@RequiresApi(16)
inline fun Fragment.startActivityForResultResolved(intent: Intent, requestCode: Int, options: Bundle, fallback: () -> Unit): Unit =
